...
```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --list_profiles
```

Create a new research direction without editing JSON:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --create_profile
```

Check the local environment:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --doctor
```

Preview recent papers without writing files:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --profile electron_ptychography --mode daily --time_range_days 7 --max_results 20 --dry_run
```

Run and append results to Excel:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --profile electron_ptychography --mode daily --time_range_days 1 --max_results 20
```

Push a short digest through Server Chan Turbo:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --profile electron_ptychography --mode daily --notify serverchan
```

Test Server Chan Turbo:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --test_notify --notify serverchan
```

Generate annual summary:

```bash
python .claude/skills/ptychography-paper-tracker/scripts/ptychography_tracker.py --profile electron_ptychography --mode annual_summary --start_year 2024 --end_year 2026 --max_papers_per_year 50
```
...
- Excel library: `.claude/skills/Ptychography_论文全量库.xlsx`
- Markdown summary: `.claude/skills/Ptychography论文解析汇总.md`
- Daily reports: `.claude/skills/daily_reports/YYYY-MM-DD_profile.md`
- History file: `.claude/skills/ptychography-paper-tracker/paper_history.json`
- Profile config: `.claude/skills/ptychography-paper-tracker/research_profiles.json`
